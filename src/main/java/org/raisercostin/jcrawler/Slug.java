...
public class Slug {
  ...
  /**Multiple types of encodings can exist: flat, dirs, etc. Return the default one and the others to allow migrations.*/
  ...
  static public Slug contentPathInitial(String text) {
    String result = urlSanitized(text);
    String path = path(result, true) + ".tmp2";
  ...
  /**A has computed for url after sanitization (removal of fragments, but keeping query params).*/
  ...
  /**Remove fragments (#fragments). For now query params (?queryParams) are not removed.*/
  ...
  /**The url converted to a relative path. Use @ instead of ? as wget. Fragments should get discarded.*/
