...
public class RichCli {
  ...
  public static void setLoggingLevel(String category, int verbosity) {
    LoggerContext context = (LoggerContext) LoggerFactory.getILoggerFactory();
    Level logLevel = Level.INFO;
    if (verbosity >= LEVEL4_TRACE)
      logLevel = Level.TRACE;
    ...
    ch.qos.logback.classic.Logger logger = context.getLogger(category);
    logger.setLevel(logLevel);
    log.debug("Logback configured category {} with verbosity {} to level {}", category, verbosity, logLevel);
  }
}
